...
if ifGraph
    ...
    if Dim == 1
        ...
        y = zeros(size(x));
        for i=1:length(x)
            y(i) = niching_func(x(i), para.FuncID); % fitness evaluation
        end
    ...
    elseif Dim == 2
        ...
        if if_mesh
            ...
            contour(X1,X2,P_F);
            if if_sample
                plot(para.sampledata(:, 1), para.sampledata(:, 2), '.');
            end
        end
        xlabel('X1');ylabel('X2');zlabel('Fitness');title(['CEC2013 Niche Benchmark FuncID: ',num2str(para.FuncID), '  # of sample: ', num2str(para.SamNum)]);
        %     saveas(gcf, [folder_name, '\graph\Func', num2str(FuncID), '.fig']);
        %     saveas(gcf, [folder_name, '\graph\Func', num2str(FuncID), '.bmp']);
    else
